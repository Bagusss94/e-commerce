...
<div class="card card-primary card-outline">
  <div class="card-header">
    <h5 class="m-0 d-flex justify-content-between flex-wrap">
      ...
      <a href="" class="btn btn-primary" id="tambah-kategori"><i class="fas fa-plus"></i> Tambah Kategori</a>
    </h5>
  </div>
  <div class="card-body">
    <table id="example2" class="table table-bordered table-striped">
      ...
      <tbody>
        ...
        foreach ($dataKategori as $key => $value) :
          $foreign = $db->table('produk')->where('id_kategori', $value['id_kategori'])->get()->getNumRows();
        ?>
          <tr>
            <td><?= $i++; ?></td>
            ...
            <td class="text-center">
              <div class="btn-group">
                <button type="button" class="btn btn-sm btn-primary" onclick="edit('<?= $value['id_kategori']; ?>')"><i class="fa fa-edit"></i></button>
                <?php if ($foreign == 0) : ?>
                  <button type="button" class="btn btn-sm btn-danger" onclick="hapus('<?= $value['id_kategori']; ?>', '<?= $value['kategori']; ?>')"><i class="fa fa-trash-alt"></i></button>
                <?php endif; ?>
              </div>
            </td>
          </tr>
...
<script>
  ...
  function hapus(id, kategori) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location = `<?= base_url('kategori/hapus/'); ?>${id}`;
      }
    });
  }
  ...
  function edit(id) {
    $.ajax({
      url: "<?= base_url("kategori/modalForm/"); ?>" + id,
      dataType: "json",
  ...
  $('#tambah-kategori').click(function(e) {
    e.preventDefault();
    $.ajax({
      url: "<?= base_url("kategori/modalForm") ?>",
      dataType: "json",
      success: function(response) {
        if (response.data) {
          $('.viewmodal').show().html(response.data);
