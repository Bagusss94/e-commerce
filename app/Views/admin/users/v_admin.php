...
<div class="card card-primary card-outline">
  <div class="card-header">
    <h5 class="m-0 d-flex justify-content-between flex-wrap">
      ...
      <a href="<?= base_url('users/tambah_admin'); ?>" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah Admin</a>
    </h5>
  </div>
  <div class="card-body">
          ...
          <tr>
            ...
            <td class="text-center">
              <div class="btn-group">
                ...
                <!-- Jika anda login pakai akun ini, ya anda tidak bisa hapus akun ini saat itu juga -->
                <?php if ($value['id_user'] != session('LoginUser')['id_user']) : ?>
  ...
  function hapus(id, email) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location = `<?= base_url('users/hapus_admin/'); ?>${id}`;
      }
    });
  }

  function edit(id) {
    window.location = `<?= base_url('users/edit_admin/'); ?>${id}`;
  }
</script>
